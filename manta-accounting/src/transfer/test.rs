...
where
    ...
    A: Accumulator<Item = UtxoAccumulatorItem<C>, Model = UtxoAccumulatorModel<C>>,
{
    /// Builds a new [`TransferDistribution`] from `parameters`, `utxo_accumulator`, and `authorization`.
    #[inline]
    pub fn new(
    ...
    /// Builds a [`TransferDistribution`] from `parameters`, `utxo_accumulator`, and `spending_key`.
    #[inline]
...
/// Samples a new [`ToPrivate`] and builds a correctness proof for it, checking if it was
/// validated using the given `proving_context` and `authorization_context`. If the check
/// succeeds, it returns a [`TranferPost`](crate::transfer) and a [`PreSender`].
#[inline]
