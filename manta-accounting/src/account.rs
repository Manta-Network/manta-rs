...
// FIXME: tweak the secret key api so that it matches with a wallet implementation (i.e. it should
//        not necessarily be generated by an `Rng`, but something like a `SecreKeySource`
...
use manta_crypto::{
    concatenate, CommitmentScheme, ConcatBytes, ContainmentProof, EncryptedMessage,
    IntegratedEncryptionScheme, KeyPair, PseudorandomFunctionFamily, VerifiedSet,
};
use rand::{
...
#[derive(derivative::Derivative, ScaleDecode, ScaleEncode)]
#[derivative(
    Clone(bound = "C::SecretKey: Clone"),
    Copy(bound = "C::SecretKey: Copy"),
    Debug(bound = "C::SecretKey: Debug"),
    Default(bound = "C::SecretKey: Default"),
    Eq(bound = "C::SecretKey: Eq"),
    Hash(bound = "C::SecretKey: Hash"),
    PartialEq(bound = "C::SecretKey: PartialEq")
)]
    ...
    C: IdentityConfiguration,
{
    ...
    /// Generates the associated `C::Rng`, `AssetParameters<C>`, and `KeyPair<I>` for
    /// this identity.
    ///
    /// # API Note
    ///
    ...
    #[inline]
    fn rng_and_parameters_and_asset_keypair<I>(&self) -> (C::Rng, AssetParameters<C>, KeyPair<I>)
    where
    ...
    /// Generates [`AssetParameters`] and a [`KeyPair`] for assets that are used by this identity.
    #[inline]
    pub fn parameters_and_asset_keypair<I>(&self) -> (AssetParameters<C>, KeyPair<I>)
    where
        I: IntegratedEncryptionScheme<Plaintext = Asset>,
        Standard: Distribution<AssetParameters<C>>,
    {
    ...
    /// Generates a [`KeyPair`] for assets that are used by this identity.
    #[inline]
    pub fn asset_keypair<I>(&self) -> KeyPair<I>
    where
        I: IntegratedEncryptionScheme<Plaintext = Asset>,
        Standard: Distribution<AssetParameters<C>>,
    {
        ...
        VoidNumberGenerator<C>: ConcatBytes,
    {
        ...
        let void_number_commitment = self.void_number_commitment(
            commitment_scheme,
            &parameters.void_number_generator,
            &parameters.void_number_commitment_randomness,
        );
        (
            ShieldedIdentity {
                ...
                asset_public_key: asset_keypair.public,
            },
            Spend {
                identity: self,
                asset_secret_key: asset_keypair.secret,
            },
        )
    }
}
    ...
    I: IntegratedEncryptionScheme<Plaintext = Asset>,
{
    ...
    pub asset_public_key: I::PublicKey,
}
...
where
    ...
    I: IntegratedEncryptionScheme<Plaintext = Asset>,
{
    /// Generates a [`Utxo`] for an `asset` using the pre-computed
    /// [`void_number_commitment`](Self::void_number_commitment) and
    /// [`utxo_randomness`](Self::utxo_randomness).
    #[inline]
    pub fn utxo(&self, commitment_scheme: &C::CommitmentScheme, asset: &Asset) -> Utxo<C>
    where
        VoidNumberCommitment<C>: ConcatBytes,
    {
        generate_utxo::<C>(
    ...
    {
        Ok(Receiver {
            encrypted_asset: I::encrypt(&asset, &self.asset_public_key, rng)?,
            utxo: self.utxo(commitment_scheme, &asset),
            asset,
            utxo_randomness: self.utxo_randomness,
...
/* TODO:
/// Sender Public Data
pub struct SenderPublicData<C, S>
where
    C: IdentityConfiguration,
    S: VerifiedSet<Item = Utxo<C>>,
{
    ...
    /// UTXO Containment Public Input
    pub utxo_containment_public_input: S::Public,
}

/// Sender Circuit Data
pub struct SenderCircuitData<C, S>
where
    C: IdentityConfiguration,
    S: VerifiedSet<Item = Utxo<C>>,
{
    /// Sender Identity
    pub identity: Identity<C>,
...
/* TODO:
/// Receiver Public Data
pub struct ReceiverPublicData<C, I>
where
    C: IdentityConfiguration,
    I: IntegratedEncryptionScheme<Plaintext = Asset>,
{
...
pub struct ReceiverCircuitData<C>
where
    C: IdentityConfiguration,
{
    /// Asset
