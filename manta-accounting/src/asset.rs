...
pub struct Selection<K, Balances, Zeroes>
where
    Balances: IntoIterator<Item = (K, AssetBalance)>,
    Balances::IntoIter: ExactSizeIterator,
    Zeroes: IntoIterator<Item = K>,
    Zeroes::IntoIter: ExactSizeIterator,
{
    /// Change Amount
...
/// [`Selection`] for [`AssetMap`] Type Alias
pub type AssetMapSelection<M> =
    Selection<<M as AssetMap>::Key, <M as AssetMap>::Balances, <M as AssetMap>::Zeroes>;
...
pub trait AssetMap: Default {
    ...
    /// Asset Selection Strategy Type
    ///
    /// This type is used by [`select`](Self::select) when looking for assets in the map.
    type Strategy: SelectionStrategy<Self>;

    /// Asset Selection Iterator Type
    ///
    /// This type is returned by [`select`](Self::select) when looking for assets in the map.
    type Balances: ExactSizeIterator<Item = (Self::Key, AssetBalance)>;

    /// Asset Selection Zeroes Iterator Type
    ///
    /// This type is returned by [`select`](Self::select) when looking for assets in the map.
    type Zeroes: ExactSizeIterator<Item = Self::Key>;
...
pub trait SelectionStrategy<M>: Default
where
    M: AssetMap + ?Sized,
{
    /// Returns `true` if `selection` could have been generated by a call to [`AssetMap::select`]
    /// with `self` as the strategy.
    fn is_valid(&self, selection: &AssetMapSelection<M>) -> bool;
}
