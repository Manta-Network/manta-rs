...
pub mod test {
    ...
    /// Verifies the `proof` generated by `ProofSystem`.
    #[inline]
    pub fn proof_system_correctness<P, F, R>(
        public_parameters: &P::PublicParameters,
        into_public_input: F,
        rng: &mut R,
    ) -> Result<bool, P::Error>
    where
        ...
        P::Compiler: Clone,
        R: CryptoRng + RngCore + ?Sized,
        F: FnOnce(P::Compiler, P::Compiler) -> Result<P::Input, P::Error>,
    {
        let context_compiler = P::context_compiler();
        let proving_compiler = P::proof_compiler();
        let (proving_context, verifying_context) =
            P::compile(public_parameters, context_compiler.clone(), rng)?;
        let proof = P::prove(&proving_context, proving_compiler.clone(), rng)?;
        P::verify(
            &verifying_context,
            &into_public_input(context_compiler, proving_compiler)?,
            &proof,
        )
    }
