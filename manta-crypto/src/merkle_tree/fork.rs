...
#[derivative(
    ...
    PartialEq(bound = "LeafDigest<C>: PartialEq, InnerDigest<C>: PartialEq, M: PartialEq, L: PartialEq")
)]
    ...
    L: LeafMap<C>,
{
    ...
    #[inline]
    fn new<T>(parameters: &Parameters<C>, base: &T, leaf_digests: L) -> Option<Self>
    where
        T: Tree<C>,
        LeafDigest<C>: Clone + Default,
        InnerDigest<C>: Default,
        ...
        L: Default,
    {
        ...
        for (i, digest) in leaf_digests.leaf_digests().into_iter().enumerate() {
            partial.push_leaf_digest(parameters, Node(partial_tree_len + i), digest.clone());
        }
        Self {
            base_contribution,
            data: partial,
        ...
        L: Default,
    {
        ...
        let new_branch = Self::new_unchecked(
            parameters,
            base,
            L::from_vec(Self::extract_leaves(
                self.base_contribution,
...
#[derivative(
    ...
    Eq(bound = "T: Eq, LeafDigest<C>: Eq, InnerDigest<C>: Eq, M: Eq, L: Eq"),
    Hash(bound = "T: Hash, LeafDigest<C>: Hash, InnerDigest<C>: Hash, M: Hash, L: Hash"),
    PartialEq(
        bound = "T: PartialEq, LeafDigest<C>: PartialEq, InnerDigest<C>: PartialEq, M: PartialEq, L: PartialEq"
    )
)]
